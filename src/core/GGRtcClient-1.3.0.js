...
export default class GGRtcClient {
	...
	initSocket(callbackFunc) {
		...
		//（主叫端）收到对端的呼叫响应回复
		this._socket.on('reply', async (data) => { // 收到回复
		...
		this._socket.on("shareLocal", (data) => {
			...
			if (data.op == "share") {  // 对方开启桌面分享
			}
			else if (data.op == "unshare") {  // 对方停止桌面分享
	...
	muteVideo_remote(flag) {
		// 判断状态
		if (!this._checkIsCalling()) return;

		// 设置操作
		this._doMute_remote("video", !flag);
  ...
  _initPeer(data) {
    ...
    this._peer.onaddstream = event => {
      JsBridge.emit('writeLog', { folder: 'web', content: `座席端:获取到视频流...` });
      ...
      switch (event.stream.id) {
        case this._remotestream_id:
		  ...
		  // 配置了播放对端视频才自动播放
		  video.play(); // 苹果手机微信需要主动调用play(),不会触发onloadedmetadata,需要点击开始播放才会触发
		  console.log('ccccc:',getConfigFromBroswer(['video_enable']).video_enable)
          if(!getConfigFromBroswer(['video_enable']).video_enable) this.muteVideo_remote(true); // 浏览器设置不允许播放画面则禁视
        ...
        case this._remoteShareStream_id:
          console.log('桌面分享流');
          // console.log("this._remote_share_stream_video", this._remote_share_stream_video);
          ...
          share.play();
          this._startRemoteShareRecord();
          JsBridge.emit('writeLog', { folder: 'web', content: `终端:开始录制远端分享视频...` });
          // 如果video标签未设置autoplay，则需要下面代码
    ...
    this._peer.oniceconnectionstatechange = event => {
      ...
      /*
			...
			*/
			...
			if (this._peer && (this._peer.iceConnectionState === "disconnected"||this._peer.iceConnectionState === "closed")) {
				...
				case 'failed':
					...
					setTimeout(() => {
						if (this._peer&&this._peer.iceConnectionState !== 'connected') {
							console.error("restartIce failed! close video call! Connection state:" + this._peer.connectionState); // 断网状态下是：failed
	...
	async _onOffer(data) {
		...
		try {
			...
			else if (data.streamType == "share" && this._localShareStream_id) data.streamId = this._localShareStream_id;
	...
	_doMute_remote(type, enabled) {
		console.log("_doMute_remote", type, enabled);
		var tracks = this._remotestream.getTracks();
		tracks.forEach(item => {
			// console.log(item.kind); // video audio
			if (item.kind == type) {
				item.enabled = enabled;
	...
	_doLocalShare(shareStream) {
		...
		if (shareStream) {  // 如果客户端捕获并直接传入了分享流
  ...
  _stopRemoteShareRecord() {
    if (!this._mediaRemoteShareRecorder || this._mediaRemoteShareRecorder.state === 'inactive') return;
